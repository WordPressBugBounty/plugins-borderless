	...
	class BORDERLESS_LICENSE {
		...
		public function license_key_deactivation() {
			...
			if ( ! empty( $api_key ) && $activation_status === 'Activated' ) {
				if ( empty( $this->deactivate( $args ) ) ) { // phpcs:ignore
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) :
					...
					<div class="notice notice-error">
						<p>
							<?php
							printf(
								__( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', $this->text_domain ),
								$this->software_title,
								'<strong>' . $host . '</strong>',
		...
		public function load_settings() {
			...
			if ( $this->no_product_id ) {
				add_settings_field( 'product_id', esc_html__( 'Product', $this->text_domain ), array( $this, 'wc_am_product_id_field' ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key ); // phpcs:ignore
			}
		...
		public function wc_am_product_id_field() {
			...
			$products = array(
				'168804' => 'Borderless Pro',
                '168805' => 'Visualmodo Pass',
			);

			echo "<select id='product_id' name='" . esc_attr( $this->wc_am_product_id ) . "'>";

			foreach ( $products as $value => $label ) {
				echo '<option value="' . esc_attr( $value ) . '" ' . selected( (string) $product_id, (string) $value, false ) . '>' . esc_html( $label ) . '</option>';
			}
		...
		public function check_for_update() {
			...
			if ( $this->plugin_or_theme === 'plugin' ) {
				...
				add_filter( 'pre_set_site_transient_update_themes', array( $this, 'update_check' ) ); // phpcs:ignore
